...
static int find_lines(Mat* frame, Mat camera_matrix, Mat dist_coeffs, Mat rvec, Mat tvec, vector<Point2f> corners)
{
  ...
  if(linesP.size() == 2)
  {
    *frame = clonius.clone();
    ...
    for(int i=0; i<1; i++)
    {
      ...
      cout << "profit: " << to3D*zero<< endl;
      
      vector<Point3f> bubi;
      bubi.push_back(Point3f(to3D*zero));
  ...
  //    if(bubi[0].x <0) bubi[0].x=0;
  //    if(bubi[0].y <0) bubi[0].y=0;
  //    if(bubi[0].z <0) bubi[0].z=0;
  //    projectPoints(bubi, rvec, tvec, camera_matrix, dist_coeffs, image_points, noArray(), 0);
...
//  VideoWriter writie("../saved_images/vid.mp4", CV_FOURCC('X','2','6','4'), 20, cvSize(cap.get(CV_CAP_PROP_FRAME_WIDTH), cap.get(CV_CAP_PROP_FRAME_HEIGHT)));
  ...
  while(1)
  {
    ...
    find_lines(&frame, camera_matrix, dist_coeffs, rvec, tvec, corners);
